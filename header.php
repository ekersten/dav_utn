<header>
    <div class="row">
        <div class="column medium-4">
            <a href="index.php"><img src="img/logo_dav.png" alt="Departamento de Aprendizaje Visual" width="230"/></a>
        </div>
        <div class="column medium-8">
            <nav>
                <ul>
                    <li><a href="quienes_somos.php">Quiénes<br/>Somos</a></li>
                    <li>
                        <a href="#">Programas para<br/>Escuelas</a>
                        <ul>
                            <li><a href="digital-junior-informacion.php">Digital Junior</a></li>
                            <li><a href="ciudadania-informacion.php">Ciudadanía Digital</a></li>
                            <li><a href="escuela-informacion.php">Escuela 2.0</a></li>
                        </ul>
                    </li>
                    <li>
                        <a href="#">Programas para Organismos<br/>Públicos y R.S.E.</a>
                        <ul>
                            <li><a href="modelo1a1.php">Modelo 1 a 1</a></li>
                            <li><a href="entropia.php">Entropía</a></li>
                            <li><a href="profesores-futuro.php">Profesores para el Futuro</a></li>
                            <li><a href="nativos-digitales.php">Nativos Digitales</a></li>
                            
                        </ul>
                    </li>
                    <li><a href="prensa.php">Prensa</a></li>
                    <li><a href="contacto.php">Contacto</a></li>
                </ul>
            </nav>
        </div>
    </div>
</header>